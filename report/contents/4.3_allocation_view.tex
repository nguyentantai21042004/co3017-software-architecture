...
\noindent \textbf{Sơ đồ Triển khai Chi tiết (Enhanced Deployment Diagram)}
...
\noindent \textbf{Chi tiết các Layer trong Kiến trúc:}
...
\noindent \textbf{Network Topology và Security Zones:}

\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{VLAN 10 -- DMZ}: Load Balancer, chỉ expose port 443 (HTTPS)
    \item \textbf{VLAN 20 -- Application}: Kubernetes worker nodes, internal communication
    \item \textbf{VLAN 30 -- Data}: Database clusters, message broker, cache
    \item \textbf{VLAN 40 -- Management}: Monitoring stack, Kubernetes master nodes
\end{itemize}

\noindent \textbf{High Availability Configuration:}

\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Kubernetes Masters}: 3 nodes với etcd cluster (quorum-based)
    \item \textbf{PostgreSQL}: Patroni với 1 Primary + 2 Replicas (streaming replication)
    \item \textbf{RabbitMQ}: 3-node cluster với mirrored queues
    \item \textbf{Redis}: Sentinel với 1 Master + 2 Replicas + 3 Sentinels
    \item \textbf{Application Pods}: HPA (Horizontal Pod Autoscaler) với min/max replicas
\end{itemize}
