...
\indentpar \indentpar Lưu ý quan trọng: Góc nhìn này mô tả Kiến trúc Đích (Target Architecture - Phase 3) của hệ thống ITS khi triển khai trên môi trường Production quy mô lớn (On-Premise Kubernetes).
\indentpar Đối với giai đoạn MVP (Phase 1) hiện tại, hệ thống được triển khai đơn giản hóa bằng Docker Compose trên một máy chủ vật lý duy nhất để phục vụ phát triển và kiểm thử nhanh (xem chi tiết tại Mục~6).
...
\noindent Phân vùng Mạng (Network Segmentation)

\indentpar \indentpar Vì đây là môi trường On-Premise, việc phân vùng mạng (segmentation) là cực kỳ quan trọng:
\begin{itemize}[leftmargin=0.7cm]
    \item DMZ Layer: Load Balancer + API Gateway.
    \item Application Layer: các pod Java/Go.
    \item Data Layer: PostgreSQL, MongoDB, RabbitMQ, Redis.
    \item Monitoring Layer: Prometheus, Grafana, Loki.
\end{itemize}
...
\noindent Sơ đồ Triển khai Chi tiết (Enhanced Deployment Diagram)

\indentpar \indentpar Sơ đồ dưới đây mô tả chi tiết hơn về kiến trúc triển khai, bao gồm các thành phần Kubernetes, network topology, và data flow giữa các layer.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/enhanced_deployment.png}
    \caption{Sơ đồ Triển khai Chi tiết với Kubernetes và Infrastructure}
    \label{fig:enhanced-deployment}
\end{figure}

\noindent Chi tiết các Layer trong Kiến trúc:

\renewcommand{\arraystretch}{1.3}
\begin{table}[H]
    \centering
    \small
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}m{2.5cm}|>{\noindent\justifying\arraybackslash}X|>{\noindent\justifying\arraybackslash}p{4cm}|}
        \hline
        \textbf{Layer}   & \textbf{Thành phần}                                                & \textbf{Chức năng}                                   \\
        \hline
        DMZ Layer        & HAProxy/NGINX Load Balancer                                        & SSL termination, rate limiting, WAF, reverse proxy   \\
        \hline
        Ingress Layer    & NGINX Ingress Controller, Cert Manager                             & Routing, TLS certificates, path-based routing        \\
        \hline
        Node Pool A      & Go Services (API Gateway, Scoring, Learner Model, Adaptive Engine) & Compute-optimized workloads, low memory footprint    \\
        \hline
        Node Pool B      & Java Services (Content, Auth, User Management)                     & Memory-optimized workloads, Spring Boot applications \\
        \hline
        Data Layer       & PostgreSQL HA, RabbitMQ Cluster, Redis Sentinel                    & Persistent storage, message queuing, caching         \\
        \hline
        Monitoring Layer & Prometheus, Grafana, Loki, Alertmanager, Jaeger                    & Metrics, logging, tracing, alerting                  \\
        \hline
        Storage Layer    & Rook-Ceph/Longhorn, SAN/NAS                                        & Persistent volumes, backup storage                   \\
        \hline
    \end{tabularx}
    \caption{Chi tiết các Layer trong Kiến trúc Triển khai}
    \label{tab:deployment-layers}
\end{table}
\renewcommand{\arraystretch}{1.0}

\noindent Network Topology và Security Zones:

\begin{itemize}[leftmargin=0.7cm]
    \item VLAN 10 -- DMZ: Load Balancer, chỉ expose port 443 (HTTPS)
    \item VLAN 20 -- Application: Kubernetes worker nodes, internal communication
    \item VLAN 30 -- Data: Database clusters, message broker, cache
    \item VLAN 40 -- Management: Monitoring stack, Kubernetes master nodes
\end{itemize}

\noindent High Availability Configuration:

\begin{itemize}[leftmargin=0.7cm]
    \item Kubernetes Masters: 3 nodes với etcd cluster (quorum-based)
    \item PostgreSQL: Patroni với 1 Primary + 2 Replicas (streaming replication)
    \item RabbitMQ: 3-node cluster với mirrored queues
    \item Redis: Sentinel với 1 Master + 2 Replicas + 3 Sentinels
    \item Application Pods: HPA (Horizontal Pod Autoscaler) với min/max replicas
\end{itemize}
...
\noindent 1. Load Balancer (Lớp DMZ)
...
\noindent 3. Observability Stack (AC9 -- On-Premise)

\indentpar \indentpar Toàn bộ stack giám sát được cài trong namespace \texttt{its-monitoring}:
\begin{itemize}[leftmargin=0.7cm]
    \item Prometheus -- metrics (scrape từ pods + node exporter)
    \item Loki -- collector logs
    \item Grafana -- dashboard hợp nhất
    \item Alertmanager -- cảnh báo nội bộ (email/SMS/Zalo OA)
    \item OpenTelemetry Collector -- gửi trace từ Java/Go đến Tempo/Jaeger
    \item Node Exporter -- metrics phần cứng của mỗi server
\end{itemize}
