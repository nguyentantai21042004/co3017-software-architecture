...
\noindent \textbf{1. Đăng ký Người dùng và Nhập môn (User Registration and Onboarding)}

\indentpar \indentpar Luồng này (tương ứng với UC-01) mô tả cách một người dùng mới (Learner) tạo tài khoản. Nó sử dụng một mô hình bất đồng bộ (event-driven) để tách rời việc tạo thông tin xác thực (AuthN) khỏi việc tạo hồ sơ người dùng (PII). Điều này tuân thủ ADR-6 (Auth Service riêng biệt) và ADR-7 (Tách PII).
...
\caption{Luồng Đăng ký Người dùng và Nhập môn}
\label{fig:user-registration-flow}
\end{figure}

\noindent \textbf{2. Cung cấp Nội dung Thích ứng (Adaptive Content Delivery)}
...
\noindent \textbf{3. Tạo Phản hồi Tức thì (Real-time Feedback Generation)}

\indentpar \indentpar Kịch bản này (một phần của UC-10) là luồng đồng bộ nhanh, tập trung vào việc cung cấp gợi ý (hints) ngay lập tức cho người học mà không cần nộp bài. Điều này yêu cầu hiệu năng cực cao (\textbf{AC3:} $< 500$ms), là lý do Scoring \& Feedback Service được viết bằng Go (ADR-1).

\indentpar \indentpar \textbf{Các thành phần tham gia:} Learner, Client, API Gateway, Scoring \& Feedback Service.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/real_time_feedback_sequence.png}
\caption{Luồng Tạo Phản hồi Tức thì}
\label{fig:realtime-feedback-flow}
\end{figure}

\noindent \textbf{4. Nộp và Chấm điểm Bài tập (Assessment Submission and Scoring)}
...
\caption{Luồng Tạo Báo cáo Giảng viên}
