...
AC1 -- Modularity, AC4 -- Testability, và AC7 -- Maintainability.

\subsubsection{Phân rã Hệ thống}

\indentpar \indentpar Hệ thống ITS được thiết kế theo phong cách Hybrid Microservices + Event-Driven Architecture. Việc phân rã hệ thống (System Decomposition) dựa trên Bounded Contexts của Domain-Driven Design (DDD).

Lưu ý về phạm vi MVP: Trong giai đoạn hiện tại (MVP), nhóm tập trung hiện thực các service cốt lõi liên quan trực tiếp đến luồng học tập thích ứng (Content, Scoring, Learner Model, Adaptive Engine). Các service hỗ trợ như User Management và Auth Service được mô tả trong kiến trúc đích (Target Architecture) nhưng chưa được tách thành microservice riêng biệt trong phiên bản code hiện tại; thay vào đó, các chức năng này được giả lập hoặc xử lý đơn giản để ưu tiên kiểm chứng logic nghiệp vụ chính.
...
Phân rã này cũng thể hiện rõ ``Bounded Contexts'' của DDD, cho phép mỗi service có thể triển khai, kiểm thử và mở rộng độc lập mà vẫn phối hợp thông qua event bus (RabbitMQ theo ADR-8).
...
\noindent Quy tắc Phụ thuộc (Dependency Rule)
...
\noindent a. Service Java (Content Service)

\noindent Cấu trúc thư mục
\begin{center}
...
\noindent Giải thích cấu trúc
\begin{itemize}[leftmargin=1.2em]
    \item Domain Layer (\texttt{models/}): Chứa các Entity JPA như \texttt{Question}. Đây là trung tâm của nghiệp vụ.
    \item Application Layer (\texttt{usecase/}): Chứa logic nghiệp vụ cốt lõi. \texttt{QuestionService} thực thi các quy tắc nghiệp vụ và gọi xuống Repository.
    \item Interface Adapters (\texttt{adapter/http}): Chứa Controller để xử lý HTTP request/response, đóng vai trò là cổng giao tiếp với bên ngoài.
    \item Infrastructure (\texttt{repository/}, \texttt{config/}): Triển khai các interface lưu trữ (Spring Data JPA) và cấu hình hệ thống.
\end{itemize}

\noindent b. Service Go (Scoring, Learner Model, Adaptive Engine)

Các service viết bằng Go tuân thủ Standard Go Project Layout, tối ưu cho microservices.

\noindent Cấu trúc thư mục (Ví dụ: Scoring Service)
\begin{center}
    \begin{minipage}{0.95\textwidth}
        \begin{verbatim}
sources/scoring/
...
\noindent Giải thích cấu trúc
\begin{itemize}[leftmargin=1.2em]
    \item cmd/api: Chứa hàm \texttt{main}, nơi khởi tạo ứng dụng, kết nối DB và đăng ký các route.
    \item internal/: Chứa mã nguồn riêng tư của service, không thể import từ bên ngoài.
          \begin{itemize}
              \item model: Định nghĩa các struct dữ liệu (Entity).
              \item repository: Tương tác trực tiếp với cơ sở dữ liệu.
              \item usecase: Chứa logic nghiệp vụ, gọi repository để lấy dữ liệu.
              \item delivery: Xử lý HTTP request, parse JSON và gọi usecase.
          \end{itemize}
\end{itemize}
...
\indentpar \indentpar Tuân thủ nguyên tắc Database-per-Service của kiến trúc Microservices, hệ thống không sử dụng một cơ sở dữ liệu khổng lồ dùng chung. Thay vào đó, mỗi service sở hữu một lược đồ dữ liệu (schema) riêng biệt, tối ưu cho nghiệp vụ của nó.

\noindent Chú thích trạng thái ERD:
\begin{itemize}[leftmargin=0.7cm]
    \item [MVP]: Đã triển khai trong phiên bản hiện tại
    \item [Target]: Kế hoạch cho kiến trúc đích, chưa triển khai
    \item [MVP + Target]: Một phần đã triển khai, phần còn lại là kế hoạch
\end{itemize}

\noindent Tổng quan MVP Database Schema:
...
\noindent 1. User Management Service (PostgreSQL) -- [Target Architecture]
...
\noindent 2. Content Service (PostgreSQL + JSONB) -- [MVP + Target]
...
\noindent 3. Learner Model Service (PostgreSQL) -- [MVP + Target]
