...
\noindent\textbf{Nhu cầu thị trường}
...
\noindent\textbf{Đối tượng người dùng}
\begin{itemize}
...
\noindent\textbf{Tiêu chí thành công}
\begin{itemize}
...
\noindent\textbf{Hệ thống hiện có và tích hợp:}
\begin{itemize}
    \item Hệ thống quản lý học tập (LMS) hiện có thông qua API hoặc LTI.
    \item Dịch vụ xác thực người dùng (Auth Service) dùng JWT/OAuth2.
    \item Hệ thống lưu trữ nội dung học tập (MinIO, CDN, PostgreSQL, Redis).
    \item Môi trường triển khai Kubernetes cho phép auto-scaling và CI/CD.
\end{itemize}

\noindent\textbf{Ràng buộc công nghệ}
\begin{itemize}
    \item Ngôn ngữ chính: Java (Spring Boot) cho backend nghiệp vụ, Golang cho các dịch vụ hiệu năng cao (AI/Adaptive Engine), và Python cho thành phần chấm điểm (Scoring/Feedback).
    \item Cơ sở dữ liệu: PostgreSQL (quan hệ) cho dữ liệu người học và nội dung; Redis cho cache hiệu năng; Kafka hoặc RabbitMQ cho giao tiếp sự kiện bất đồng bộ.
    \item Kiến trúc: Microservices + Event-Driven, áp dụng Clean Architecture để tách biệt domain, application và infrastructure.
    \item Áp dụng nguyên tắc SOLID, DDD (Domain-Driven Design) và Twelve-Factor App cho thiết kế module độc lập và testable.
\end{itemize}

\noindent\textbf{Kỳ vọng hiệu năng và khả năng mở rộng}
\begin{itemize}
    ...
    \item Khả năng quan sát (Observability): theo dõi hệ thống bằng Prometheus, Grafana, Jaeger, ELK Stack, cho phép phát hiện sự cố và phân tích hành vi người học theo thời gian thực.
\end{itemize}
