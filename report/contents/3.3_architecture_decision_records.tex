...
\indentpar \indentpar Sau khi lựa chọn kiến trúc tổng thể là Hybrid Microservices + Event-Driven, bước tiếp theo là ghi lại chi tiết các quyết định kỹ thuật quan trọng giúp hiện thực hóa kiến trúc đó.
Phần này sử dụng mô hình Architecture Decision Record (ADR) để mô tả bối cảnh, quyết định, lý do và hệ quả theo cách có thể truy vết (traceable) tới các yêu cầu nghiệp vụ và các đặc tính kiến trúc (ACs).
...
\begin{table}[H]
    \centering
    \small
    \begin{tabularx}{\textwidth}{|A{1.5cm}|B|A{2.2cm}|A{2cm}|}
        \hline
        \textbf{ADR}\rule{0pt}{1.6em} & \textbf{Mục tiêu chính}                                        & \textbf{Liên kết ACs} & \textbf{Trạng thái} \\
        \hline
        ADR-1                         & Xác định chiến lược ngôn ngữ đa nền tảng (Polyglot)            & AC3, AC7              & MVP                 \\
        \hline
        ADR-2                         & Lựa chọn cơ sở dữ liệu quan hệ chính (PostgreSQL)              & AC6, AC7              & MVP                 \\
        \hline
        ADR-3                         & Thiết lập mô hình kiến trúc nội bộ (Clean/Hexagonal)           & AC1, AC4              & MVP                 \\
        \hline
        ADR-4                         & Triển khai Repository Pattern để tách tầng dữ liệu             & AC1, AC4              & MVP                 \\
        \hline
        ADR-5                         & Định nghĩa chiến lược kiểm thử (Testing Pyramid)               & AC4, AC7              & Partial             \\
        \hline
        ADR-6                         & Thiết kế kiến trúc bảo mật tập trung (AuthN/AuthZ)             & AC6                   & Target              \\
        \hline
        ADR-7                         & Chiến lược bảo vệ dữ liệu cá nhân (Data Privacy \& Compliance) & AC6                   & Target              \\
        \hline
        ADR-8                         & Lựa chọn Message Broker cho giao tiếp bất đồng bộ              & AC1, AC2, AC3         & MVP                 \\
        \hline
        ADR-9                         & Chiến lược nhất quán dữ liệu phân tán (Saga Pattern)           & AC1, AC6              & Target              \\
        \hline
        ADR-10                        & Chiến lược quan sát hệ thống (Observability Strategy)          & AC9                   & Target              \\
        \hline
    \end{tabularx}
...
\noindent\textit{Chú thích trạng thái: MVP = Đã triển khai trong phiên bản hiện tại, Partial = Triển khai một phần, Target = Kế hoạch cho kiến trúc đích.}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \hline
        Bối cảnh                  & \begin{itemize}[leftmargin=0.4cm]
                  ...
                  \end{itemize}
                                             ...
                                             \end{itemize}                                                                                                          \\
        \hline
        Quyết định                & \begin{itemize}[leftmargin=0.4cm]
                                             ...
                                             \end{itemize} \\
        \hline
        Lý luận                   & \begin{itemize}[leftmargin=0.4cm]
        ...
        \end{itemize}                                                                                                                                      \\
        \hline
        Rủi ro                    & \begin{itemize}[leftmargin=0.4cm]
                                             ...
                                             \end{itemize}                                                              \\
        \hline
        Các lựa chọn              & \begin{itemize}[leftmargin=0.4cm]
                                             ...
                                             \end{itemize}                                                                    \\
        \hline
        Liên quan                 & Ảnh hưởng tới ADR-3 (Clean Architecture cho Java/Go) và ADR-5 (Testing Pyramid cho hai stack).                                                                                     \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \textbf{Hạng mục}\rule{0pt}{1.6em} & \multicolumn{1}{c|}{\textbf{Nội dung}}                                                                                                                                              \\
        \hline
        Bối cảnh                  & `User Management` và `Content Service` yêu cầu cơ sở dữ liệu quan hệ bảo đảm ACID, hỗ trợ truy vấn phức tạp, RBAC, metadata dạng JSON và có cơ chế replication/backup trưởng thành. \\
        \hline
        Quyết định                & \begin{itemize}[leftmargin=0.4cm]
        ...
        Các lựa chọn              & \begin{itemize}[leftmargin=0.4cm]
                                             ...
                                             \end{itemize}                                                                                                       \\
        \hline
        Liên quan                 & \begin{itemize}[leftmargin=0.4cm]
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        Bối cảnh                  & \begin{itemize}[leftmargin=0.4cm]
                                             ...
                                             \end{itemize}                                \\
        \hline
        Quyết định                & \begin{enumerate}[leftmargin=0.4cm]
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \hline
        Bối cảnh                  & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Kiến trúc của hệ thống là Hybrid Microservices + Event-Driven (Mục~3.2), yêu cầu một message broker để xử lý giao tiếp bất đồng bộ.
                                                 \item Hệ thống cần truyền tải các Domain Events giữa các service (ví dụ: `SubmissionCompleted`, `LearnerModelUpdated`) giữa `ScoringService`, `AdaptiveEngine`, và `LearnerModelService`.
                                                 \item Cần một cơ chế đáng tin cậy để đảm bảo các sự kiện được gửi và nhận một cách nhất quán.
                                             \end{itemize}                                                                                                                                                   \\
        \hline
        Quyết định                & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Chọn RabbitMQ làm message broker chính cho hệ thống.
        ...
        \hline
        Lý luận                   & \begin{itemize}[leftmargin=0.4cm]
                                                 \item RabbitMQ là ``smart broker'' (broker thông minh), hỗ trợ các mô hình định tuyến phức tạp thông qua exchanges và routing keys.
        ...
        \hline
        Các lựa chọn              & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Apache Kafka: Bị từ chối vì Kafka là ``dumb log'' (log lưu trữ tuần tự) và yêu cầu ``smart consumer'' (consumer phải tự quản lý offset). Mặc dù Kafka vượt trội về thông lượng (throughput) rất cao và khả năng replay sự kiện, mô hình định tuyến phức tạp của RabbitMQ được đánh giá là phù hợp hơn với nhu cầu nghiệp vụ hiện tại.
                                                 \item NATS: Bị từ chối vì NATS tập trung vào hiệu suất cực cao và đơn giản, nhưng thiếu các cơ chế đảm bảo (durability) và định tuyến phức tạp mạnh mẽ như RabbitMQ.
                                             \end{itemize} \\
        \hline
        Liên quan                 & \begin{itemize}[leftmargin=0.4cm]
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \hline
        Quyết định                & \begin{enumerate}[leftmargin=0.4cm]
                                                 \item Áp dụng Saga Pattern để quản lý các giao dịch phân tán (distributed transactions).
                                                 \item Ưu tiên sử dụng mô hình Saga Choreography (Điều phối Phi tập trung) hơn là Orchestration.
                                                 \item Áp dụng Transactional Outbox Pattern để đảm bảo các sự kiện được gửi đi một cách đáng tin cậy (ngay cả khi service bị sập sau khi commit DB).
                                             \end{enumerate}                                                                                                                                                                                                                                                                                           \\
        \hline
        Lý luận                   & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Saga Choreography: Phù hợp tự nhiên với kiến trúc Event-Driven (Mục~3.2) mà chúng ta đã chọn. Các service sẽ phát ra các sự kiện (ví dụ: `UserDeleted`) và các service khác sẽ lắng nghe và phản ứng lại, thay vì bị điều khiển bởi một ``nhạc trưởng'' (orchestrator) trung tâm. Điều này giúp giảm coupling và tránh một điểm lỗi đơn (Single Point of Failure).
                                                 \item Transactional Outbox Pattern: Để tránh kịch bản ``dual-write'' (lỗi khi commit DB thành công nhưng gửi event thất bại), service sẽ ghi nghiệp vụ (ví dụ: xóa user) và ghi sự kiện (event) vào một bảng ``Outbox'' trong cùng một giao dịch (transaction) CSDL. Một tiến trình riêng (ví dụ: Debezium hoặc một worker) sẽ đọc từ bảng Outbox này và gửi sự kiện đến RabbitMQ (ADR-8). Điều này đảm bảo tính nhất quán ``At-Least-Once''.
        ...
        \hline
        Các lựa chọn              & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Saga Orchestration: Bị từ chối (cho hiện tại) vì nó yêu cầu một service ``Orchestrator'' trung tâm, làm tăng độ phức tạp và tạo ra một điểm nghẽn (bottleneck) tiềm ẩn.
                                                 \item Two-Phase Commit (2PC): Bị từ chối vì đây là một giao thức đồng bộ (blocking), vi phạm nguyên tắc performance và khả năng phục hồi (resilience) của microservices.
                                             \end{itemize}                                                                                                                                                                                                                                                                 \\
        \hline
        Liên quan                 & \begin{itemize}[leftmargin=0.4cm]
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \hline
        Bối cảnh                  & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Hệ thống là một hệ thống phân tán, bất đồng bộ và đa ngôn ngữ (Polyglot: Java + Go, ADR-1).
                                                 \item Để đáp ứng AC9: Observability, việc gỡ lỗi (debug) một request (ví dụ: ``tại sao việc nộp bài bị chậm?'') là cực kỳ phức tạp, vì nó đi qua API Gateway (Go), `ScoringService` (Go), `RabbitMQ`, và `LearnerModelService` (Go/Java).
                                                 \item Đã xác định các công cụ (Prometheus, Grafana, Loki), nhưng cần các tiêu chuẩn (standards) để chúng hoạt động hiệu quả.
                                             \end{itemize}                            \\
        \hline
        Quyết định                & \begin{enumerate}[leftmargin=0.4cm]
                                                 \item Distributed Tracing (Truy vết Phân tán): Bắt buộc triển khai. Mọi request vào hệ thống (tại API Gateway) phải được gán một Trace ID (hoặc Correlation ID) duy nhất.
                                                 \item Propagation (Lan truyền): Trace ID này bắt buộc phải được lan truyền (propagated):
                                                       \begin{itemize}[nosep]
                      \item Qua các HTTP headers trong các cuộc gọi service-to-service (ví dụ: Go gọi Java).
                      \item Qua các Message Headers của RabbitMQ (ADR-8) cho các flow bất đồng bộ.
                  \end{itemize}
                                                 \item Structured Logging (Logging có Cấu trúc): Tất cả các service (cả Java và Go) bắt buộc phải ghi log dưới định dạng JSON.
                                                 \item Log Content: Mọi dòng log JSON bắt buộc phải chứa \texttt{Trace ID} (từ mục 1) để liên kết các log của nhiều service về cùng một request.
                                             \end{enumerate}                                                                                   \\
        \hline
        Lý luận                   & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Distributed Tracing: Cho phép tái tạo lại toàn bộ hành trình của một request, xác định service nào bị chậm hoặc gây ra lỗi. Sử dụng công cụ như Jaeger hoặc OpenTelemetry.
                                                 \item Structured Logging (JSON): Cho phép các công cụ như Loki phân tích (parse) và lập chỉ mục (index) log một cách hiệu quả. Thay vì tìm kiếm text (ví dụ: \texttt{grep}), chúng ta có thể query (ví dụ: \texttt{SELECT * WHERE trace\_id == 'abc-123'}).
                                                 \item Sự kết hợp này là cách duy nhất để đáp ứng AC9: Observability trong một kiến trúc phức tạp và polyglot.
        ...
        \hline
        Các lựa chọn              & \begin{itemize}[leftmargin=0.4cm]
                                                 \item Logging văn bản thuần túy (Plain-text logging): Bị từ chối. Mặc dù dễ đọc trong môi trường development local, nó hoàn toàn vô dụng trong một hệ thống phân tán, không thể phân tích và truy vấn tập trung.
                                                 \item Chỉ dùng Metrics (Prometheus) mà không có Tracing: Bị từ chối vì metrics chỉ cho biết ``có vấn đề'' nhưng không cho biết ``vấn đề ở đâu'' trong một request cụ thể.
                                             \end{itemize}                                                     \\
        \hline
        Liên quan                 & \begin{itemize}[leftmargin=0.4cm]
