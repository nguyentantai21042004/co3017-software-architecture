...
\noindent\textit{Chú thích trạng thái: \textbf{MVP} = Đã triển khai trong phiên bản hiện tại, \textbf{Partial} = Triển khai một phần, \textbf{Target} = Kế hoạch cho kiến trúc đích.}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|D{2.4cm}|E|}
        ...
        \textbf{Bối cảnh}                  & \begin{itemize}[leftmargin=0.4cm]
                                                 ...
                                                 \item Để đáp ứng \textbf{AC9: Observability}, việc gỡ lỗi (debug) một request (ví dụ: ``tại sao việc nộp bài bị chậm?'') là cực kỳ phức tạp, vì nó đi qua API Gateway (Go), `ScoringService` (Go), `RabbitMQ`, và `LearnerModelService` (Go/Java).
