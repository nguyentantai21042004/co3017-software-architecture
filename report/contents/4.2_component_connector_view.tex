...
Phần này đặc biệt quan trọng vì nó hiện thực hóa các quyết định trong các ADR như ADR-1, ADR-3, ADR-6 và ADR-7. Đồng thời, phần này cũng trực tiếp thể hiện các ACs trọng yếu: AC2, AC3 và AC6.
...
\noindent Chi tiết các Component và Interfaces:
...
\noindent Trạng thái Triển khai Components:

\begin{table}[H]
    \centering
    \small
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{2.5cm}|>{\noindent\justifying\arraybackslash}X|}
        \hline
        \textbf{Component}    & \textbf{Trạng thái} & \textbf{Ghi chú}          \\
        \hline
        Content Service       & MVP                 & Đã triển khai đầy đủ      \\
        \hline
        Scoring Service       & MVP                 & Đã triển khai đầy đủ      \\
        \hline
        Learner Model Service & MVP                 & Đã triển khai đầy đủ      \\
        \hline
        Adaptive Engine       & MVP                 & Đã triển khai đầy đủ      \\
        \hline
        API Gateway           & Target              & Chưa triển khai trong MVP \\
        \hline
        Auth Service          & Target              & Chưa triển khai trong MVP \\
        \hline
        User Management       & Target              & Chưa triển khai trong MVP \\
        \hline
    \end{tabularx}
    \caption{Trạng thái triển khai các Components}
    \label{tab:component_status}
\end{table}

\noindent Message Queue Channels (RabbitMQ):
...
\indentpar \indentpar Sau khi hệ thống được phân tách thành nhiều module và microservice độc lập, cần mô tả rõ cách các thành phần này giao tiếp với nhau. Trong giai đoạn MVP, nhóm ưu tiên sự đơn giản và tốc độ phát triển, nhưng vẫn giữ nguyên tắc phân tách rõ ràng.

\noindent a. Giao tiếp Đồng bộ (Synchronous Communication)

Sử dụng cho các tương tác yêu cầu-phản hồi (request-response) ngay lập tức.
...
\begin{itemize}[leftmargin=1.5em]
    \item Giao thức: REST qua HTTP. Tất cả các giao tiếp giữa Client-Service và Service-Service (ví dụ: Adaptive Engine gọi Content Service) đều sử dụng chuẩn RESTful API với định dạng JSON.
    \item Lý do chọn REST cho MVP: Xem phần Phân tích Đánh đổi bên dưới.
\end{itemize}

\noindent b. Phân tích Đánh đổi: REST vs gRPC

\indentpar \indentpar Mặc dù gRPC mang lại hiệu năng cao hơn và type-safety tốt hơn (như đã đề cập trong thiết kế ban đầu), nhóm quyết định sử dụng REST/HTTP cho phiên bản hiện tại vì các lý do sau:

\begin{itemize}[leftmargin=1.5em]
    \item Tốc độ phát triển: REST là tiêu chuẩn quen thuộc, giúp đội ngũ phát triển nhanh chóng tích hợp và gỡ lỗi (debug) trực tiếp bằng các công cụ như Postman hay cURL mà không cần tooling phức tạp.
    \item Giảm độ phức tạp: Loại bỏ nhu cầu quản lý file `.proto` và quy trình code generation trong giai đoạn đầu, giúp tập trung vào logic nghiệp vụ cốt lõi.
    \item Tính tương thích: JSON/HTTP được hỗ trợ tự nhiên bởi cả Spring Boot (Java), Gin (Go) và Next.js (Frontend), giảm thiểu rủi ro tích hợp.
\end{itemize}
...
\noindent b. Giao tiếp Bất đồng bộ (Asynchronous Communication)
...
\noindent Luồng 1: Học tập Thích ứng (UC-08: Adaptive Learning Flow)
...
\noindent Luồng 2: Chấm điểm Bất đồng bộ (UC-10: Asynchronous Scoring)
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Scoring Service (Luồng đồng bộ):
          \begin{enumerate}
...
\noindent Các Giai đoạn của AI Pipeline:

\begin{enumerate}[leftmargin=1.5em]
    \item Input Layer (Lớp Đầu vào):
    ...
    \item Scoring Pipeline (Chấm điểm):
          \begin{itemize}[nosep,leftmargin=0.9cm]
    ...
    \item Learner Model Pipeline (Mô hình hóa Người học):
          \begin{itemize}[nosep,leftmargin=0.9cm]
    ...
    \item Adaptive Engine Pipeline (Chọn Nội dung Thích ứng):
          \begin{itemize}[nosep,leftmargin=0.9cm]
...
\noindent Thuật toán AI/ML được sử dụng:
