...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{SRP}: Lớp này có nhiều hơn một lý do để thay đổi không? (Ví dụ: Thay đổi nghiệp vụ VÀ thay đổi CSDL?)
    \item \textbf{OCP}: Nếu thêm một tính năng tương tự (ví dụ: loại gợi ý mới), chúng ta phải sửa code ở đây hay thêm code mới? (Phải là ``thêm'')
    \item \textbf{LSP}: Lớp con này có vi phạm bất kỳ hợp đồng nào của lớp cha không? (Ví dụ: Ném \texttt{UnsupportedOperationException}?)
    \item \textbf{ISP}: Interface này có ``béo'' không? Client sử dụng nó có cần tất cả các phương thức không?
    \item \textbf{DIP}: Module cấp cao này có import bất kỳ thứ gì từ tầng Infrastructure (ví dụ: \texttt{Postgres...}) không? (Không được phép)
    \item \textbf{Tests}: Unit test có sử dụng Mocks/Stubs không? (Xác minh DIP)
\end{itemize}
