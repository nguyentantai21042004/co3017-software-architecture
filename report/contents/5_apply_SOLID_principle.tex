...
% \indentpar \indentpar Chương này mô tả chi tiết cách 5 nguyên tắc SOLID được áp dụng trong toàn bộ hệ thống ITS. Việc tuân thủ này là nền tảng kỹ thuật bắt buộc để đạt được các Đặc tính Kiến trúc (ACs) đã ưu tiên, đặc biệt là \textbf{AC1: Modularity}, \textbf{AC4: Testability}, và \textbf{AC7: Maintainability}.

% \subsection{Nguyên tắc Trách nhiệm Đơn lẻ (Single Responsibility Principle - SRP)}

% \subsubsection{Tổng quan Nguyên tắc}

% \noindent\textbf{Định nghĩa}: ``Một lớp (class) chỉ nên có một, và chỉ một, lý do để thay đổi.''

% \noindent\textbf{Áp dụng trong ITS}: Nguyên tắc này được áp dụng ở nhiều cấp độ:

% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Cấp độ Dịch vụ (Service)}: Mỗi microservice sở hữu một miền nghiệp vụ (Bounded Context) duy nhất. Ví dụ, \texttt{UserManagementService} chỉ có lý do thay đổi khi nghiệp vụ quản lý người dùng thay đổi; nó không thay đổi khi thuật toán chấm điểm (Scoring) thay đổi
%     \item \textbf{Cấp độ Lớp (Class)}: Mỗi lớp chỉ có một trách nhiệm. Ví dụ, \texttt{ScoringUseCase} chịu trách nhiệm điều phối nghiệp vụ chấm điểm, trong khi \texttt{ScoringRepository} chịu trách nhiệm lưu trữ kết quả
%     \item \textbf{Cấp độ Phương thức (Method)}: Mỗi phương thức thực hiện tốt một việc duy nhất
% \end{itemize}

% \subsubsection{Ví dụ Triển khai}

% Như được minh họa trong tài liệu SOLID principles, chúng ta đã tách biệt rõ ràng các trách nhiệm.

% \noindent\textbf{Ví dụ 1: Tách biệt Service (Vi phạm và Sửa lỗi)}
...
% // ✅ TỐT: Tách thành các service riêng biệt
...
% \noindent\textbf{Ví dụ 2: Tách biệt Tầng trong Service (Clean Architecture)}
...
% // ❌ XẤU: Controller (lớp Web) làm tất cả mọi việc
% type QuizController struct {
%     db *sql.DB
% }

% func (c *QuizController) SubmitAnswer(ctx *gin.Context) {
%     // ... (Trách nhiệm 1: Xử lý HTTP)
    
%     // ... (Trách nhiệm 2: Logic nghiệp vụ)
%     score := c.calculateScore(req.Answer) 
    
%     // ... (Trách nhiệm 3: Truy cập CSDL)
...
% \begin{itemize}[leftmargin=1.5em]
%     \item[\CheckedBox] Mỗi microservice có một Bounded Context nghiệp vụ rõ ràng
%     \item[\CheckedBox] Các lớp (class) chỉ có một lý do duy nhất để thay đổi (ví dụ: UseCase thay đổi khi nghiệp vụ thay đổi, Repository thay đổi khi CSDL thay đổi)
%     \item[\Square] Các phương thức (method) được giữ ngắn gọn (dưới 20 dòng) và tập trung
%     \item[\CheckedBox] Không có ``god class'' (lớp thần thánh) hoặc ``utility class'' (lớp tiện ích chứa rác)
% \end{itemize}
...
% \noindent\textbf{Định nghĩa}: ``Các thực thể phần mềm (lớp, module, hàm) nên được mở để mở rộng, nhưng đóng để sửa đổi.''

% \noindent\textbf{Chiến lược}: Chúng ta đạt được OCP bằng cách phụ thuộc vào abstractions (interfaces). Các ``policy'' (chính sách) cấp cao được đóng, nhưng chúng ta có thể thêm các ``plugin'' (triển khai) mới để mở rộng hành vi.

% \noindent\textbf{Mẫu Thiết kế sử dụng}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Strategy Pattern}: Dùng cho các thuật toán có thể thay thế (ví dụ: thuật toán chấm điểm, thuật toán gợi ý)
%     \item \textbf{Dependency Injection}: Dùng để ``tiêm'' các chiến lược (strategy) khác nhau vào lúc runtime
% \end{itemize}

% \subsubsection{Ví dụ Code}

% Đây là ví dụ quan trọng nhất để đáp ứng FR12: Live AI Model Swapping và được minh họa chi tiết trong tài liệu SOLID principles.

% \noindent\textbf{Ví dụ: Chiến lược Gợi ý Mở rộng (Strategy Pattern)}
...
% // ❌ XẤU: Đóng để mở rộng. Phải sửa đổi code khi thêm loại mới.
...
%         // ...
%     }
%     // Phải sửa file này để thêm "example"
...
% \noindent\textbf{Định nghĩa}: ``Các đối tượng của lớp con phải có thể thay thế được cho các đối tượng của lớp cha mà không làm thay đổi tính đúng đắn của chương trình.''

% \noindent\textbf{Áp dụng trong ITS}: Chúng ta đảm bảo LSP bằng cách tuân thủ ``hợp đồng'' (contract) của interface hoặc lớp cha. Các lớp con có thể mở rộng hành vi, nhưng không được vi phạm các giả định của lớp cha.

% \noindent\textbf{Ví dụ trong ITS}:
% \begin{itemize}[leftmargin=1.5em]
...
% // Lớp cha giả định *tất cả* đều có thể tự động chấm điểm
...
%     @Override
%     public double autoScore(String answer) {
%         // Dự án không thể tự chấm điểm
%         throw new UnsupportedOperationException("Phải chấm thủ công"); 
%     }
% }
...
%             // Lỗi! Ném exception khi gặp ProjectAssessment
...
% // ✅ TỐT: Tuân thủ LSP bằng cách tách interface
...
% // Project tuân thủ hợp đồng "Manual"
...
%             // An toàn! Tất cả đều hỗ trợ autoScore()
%             a.autoScore(userAnswer);
%         }
%     }
% }
...
% \noindent\textbf{Định nghĩa}: ``Client không nên bị buộc phải phụ thuộc vào các phương thức (trong interface) mà chúng không sử dụng.''

% \noindent\textbf{Áp dụng trong ITS}: Chúng ta tránh các ``fat interface'' (interface béo phì). Thay vào đó, chúng ta tạo ra các interface nhỏ, tập trung (role-based interfaces).

% \noindent\textbf{Áp dụng trong}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Repository}: Tách biệt Read (Đọc) và Write (Ghi). Đây là nền tảng của mô hình CQRS
%     \item \textbf{Service}: Tách biệt các nghiệp vụ khác nhau
% \end{itemize}
...
% // Vấn đề: `ScoringService` chỉ cần `GetSkillMastery` và `UpdateMasteryScore`
% // nhưng bị buộc phải "biết" về Profile và Report.
...
% // ✅ TỐT: Các interface nhỏ, tập trung (role-based)

% // Interface cho việc đọc Profile
% type ProfileReader interface {
%     GetProfile(id string) (*Profile, error)
% }

% // Interface cho việc đọc/ghi Skill
...
% // Lớp triển khai có thể thực thi nhiều interface
...
% // Client (ScoringService) chỉ phụ thuộc vào thứ nó cần
% type ScoringService struct {
%     skillReader SkillMasteryReader
%     skillWriter SkillMasteryWriter
% }

% // Client (AdaptiveEngine) chỉ phụ thuộc vào thứ nó cần
...
% \noindent\textbf{Định nghĩa}: ``Các module cấp cao (policy) không nên phụ thuộc vào các module cấp thấp (detail). Cả hai nên phụ thuộc vào abstractions (interfaces).''

% \noindent\textbf{Tác động}: Đây là nguyên tắc quan trọng nhất đối với kiến trúc của chúng ta.
% \begin{itemize}[leftmargin=1.5em]
...
% Ví dụ này cho thấy cách Tầng Application (cấp cao) phụ thuộc vào Interface, và Tầng Infrastructure (cấp thấp) cũng phụ thuộc vào Interface đó.

% \noindent\textbf{Tầng Application (Cấp cao)}: Định nghĩa Use Case và Interface nó cần.

% \begin{lstlisting}[language=Java, caption={Tầng Application - Phụ thuộc vào Abstraction}]
% // Tầng Application - Phụ thuộc vào Abstraction
...
% // High-level module (Policy) - Phụ thuộc vào Interface
% package application;

% @Service
% public class UpdateSkillMasteryUseCase {
%     // Phụ thuộc vào Abstraction, không phải Concretion
%     private final LearnerModelRepository repository; 

%     // Dependency được tiêm vào (DI)
%     @Autowired
...
%         model.updateSkill(skill, score); // Logic
%         repository.save(model); // Gọi interface
%     }
% }
% \end{lstlisting}

% \noindent\textbf{Tầng Infrastructure (Cấp thấp)}: Cung cấp triển khai (implementation) cho Interface.

% \begin{lstlisting}[language=Java, caption={Tầng Infrastructure - Cung cấp Concretion}]
% // Tầng Infrastructure - Cung cấp Concretion

% @Repository
% public class PostgresLearnerModelRepository implements LearnerModelRepository {
    
%     @Autowired
%     private JdbcTemplate jdbcTemplate; // Chi tiết triển khai
...
% DIP là chìa khóa để đạt được Testability (AC4) và mục tiêu Coverage > 80\% (ADR-5). Chúng ta có thể kiểm thử logic nghiệp vụ (UseCase) mà không cần CSDL.

% \begin{lstlisting}[language=Java, caption={Kiểm thử UseCase mà không cần CSDL}]
% // Kiểm thử UseCase mà không cần CSDL

% @Test
% public void testUpdateSkillMastery() {
%     // 1. Dựng (Mock) Abstraction (Interface)
...
%     // 3. Tiêm (Inject) Mock vào UseCase
...
%     // 5. Xác minh (Verify) rằng logic đã gọi interface đúng
%     // (Chúng ta không quan tâm CSDL làm gì, chỉ cần biết nghiệp vụ gọi 'save')
...
% Chúng tôi sử dụng các công cụ phân tích tĩnh (static analysis) để liên tục đo lường mức độ tuân thủ SOLID.

% \subsubsection{Số liệu Code}

% Các số liệu này được tích hợp vào CI/CD pipeline (ADR-5) để đảm bảo chất lượng.
...
% \hline
% \textbf{Số liệu} & \textbf{Mục tiêu} & \textbf{Công cụ} & \textbf{Trạng thái} & \textbf{Ý nghĩa (liên quan đến SOLID)} \\
% \hline
...
% Checklist này được sử dụng trong mọi Pull Request:

% \begin{itemize}[leftmargin=1.5em]
%     \item[\Square] \textbf{SRP}: Lớp này có nhiều hơn một lý do để thay đổi không? (Ví dụ: Thay đổi nghiệp vụ VÀ thay đổi CSDL?)
%     \item[\Square] \textbf{OCP}: Nếu thêm một tính năng tương tự (ví dụ: loại gợi ý mới), chúng ta phải sửa code ở đây hay thêm code mới? (Phải là ``thêm'')
%     \item[\Square] \textbf{LSP}: Lớp con này có vi phạm bất kỳ hợp đồng nào của lớp cha không? (Ví dụ: Ném \texttt{UnsupportedOperationException}?)
%     \item[\Square] \textbf{ISP}: Interface này có ``béo'' không? Client sử dụng nó có cần tất cả các phương thức không?
%     \item[\Square] \textbf{DIP}: Module cấp cao này có import bất kỳ thứ gì từ tầng Infrastructure (ví dụ: \texttt{Postgres...}) không? (Không được phép)
%     \item[\Square] \textbf{Tests}: Unit test có sử dụng Mocks/Stubs không? (Xác minh DIP)
% \end{itemize}
...
% Đánh giá kiến trúc dựa trên các kịch bản thực tế.
...
% Phân tích này xác nhận lại các đánh đổi đã được chấp nhận.

% \noindent\textbf{Sensitivity Points (Điểm nhạy cảm)}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Hiệu năng CSDL (Postgres)}: Hiệu năng của PostgreSQL ảnh hưởng trực tiếp đến các service Java. Tối ưu hóa query và indexing là rất quan trọng
%     \item \textbf{Độ trễ Mạng (Microservices)}: Kiến trúc microservice làm tăng các cuộc gọi mạng. Luồng ``Tạo Báo cáo Giảng viên'' (yêu cầu tổng hợp dữ liệu từ 3 service) là một điểm nhạy cảm về độ trễ
%     \item \textbf{Tỷ lệ Cache Hit}: Tỷ lệ cache-hit-ratio của Redis (cho PII) ảnh hưởng trực tiếp đến độ trễ của AdaptiveEngine
% \end{itemize}

% \noindent\textbf{Trade-off Points (Điểm Đánh đổi)}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Microservices Complexity vs. Modularity}: Chúng ta chấp nhận độ phức tạp vận hành cao của Microservices để đạt được Modularity (cho FR12) và Scalability (AC2)
%     \item \textbf{Eventual Consistency vs. Performance}: Chúng ta chọn mô hình Event-Driven cho việc chấm điểm. Điều này tăng hiệu năng (AC3) (phản hồi tức thì <500ms) nhưng đánh đổi bằng tính nhất quán tức thời (Learner Model được cập nhật sau vài giây)
%     \item \textbf{Polyglot Programming vs. Team Expertise}: Chúng ta chọn Polyglot (ADR-1) để tối ưu hiệu năng (Go) và bảo trì (Java). Chúng ta chấp nhận rủi ro về chi phí đào tạo và sự phức tạp của CI/CD pipeline
% \end{itemize}

% \noindent\textbf{Risks (Rủi ro)}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Operational Complexity}: Vận hành một stack polyglot (Java + Go) trên Kubernetes với RabbitMQ và nhiều CSDL (Postgres + Mongo) đòi hỏi đội ngũ DevOps rất trưởng thành
%     \item \textbf{Distributed Transaction Management}: Việc thiếu kinh nghiệm triển khai Saga pattern (để đảm bảo nhất quán dữ liệu) là rủi ro lớn nhất
%     \item \textbf{Team Learning Curve}: Đội ngũ Java cần học Go, và toàn bộ đội ngũ cần hiểu rõ Clean Architecture/DIP
% \end{itemize}

% \noindent\textbf{Non-Risks (Không phải Rủi ro)}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Technology Maturity}: Tất cả các công nghệ được chọn (Java/Spring, Go, K8s, Postgres, RabbitMQ) đều đã được chứng minh, trưởng thành và có cộng đồng lớn
%     \item \textbf{Vendor Lock-in}: Bằng cách sử dụng K8s, Postgres, RabbitMQ (tất cả đều là mã nguồn mở), chúng ta tránh được việc bị khóa chặt vào một nhà cung cấp đám mây
% \end{itemize}
...
% Việc áp dụng SOLID đã tạo ra những cải tiến đáng kể, đặc biệt khi so sánh với các dự án ``monolith'' truyền thống.

% \noindent\textbf{Cải tiến Định lượng (Ước tính)}:
...
% \hline
% \textbf{Số liệu} & \textbf{Trước SOLID (Monolith)} & \textbf{Sau SOLID (Clean Arch)} & \textbf{Cải tiến} & \textbf{Ý nghĩa} \\
% \hline
...
% \noindent\textbf{Cải tiến Định tính}:
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Dễ dàng Onboarding}: Nhờ SRP, developer mới có thể hiểu một service (ví dụ: ScoringService) mà không cần biết về UserManagementService
%     \item \textbf{Phát triển Tính năng Nhanh hơn}: Nhờ OCP, việc thêm một loại gợi ý mới là một tác vụ thêm (add), không phải sửa (modify), giảm rủi ro
%     \item \textbf{Tự tin Tái cấu trúc (Refactoring)}: Nhờ DIP và Testability (AC4), developer có thể tự tin tái cấu trúc code nghiệp vụ vì đã có bộ Unit Test bảo vệ
% \end{itemize}

% \subsubsection{Những Thách thức Gặp phải}

% \noindent\textbf{Sự phức tạp ban đầu (Initial Complexity)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Tiếp nhận của Đội ngũ (Team Adoption)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Chi phí Hiệu năng (Performance Overhead)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Clean Architecture trong Microservices}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Event-Driven cho các Thành phần AI}:
% \begin{itemize}[leftmargin=1.5em]
%     \item Quyết định tách rời (decouple) ScoringService và LearnerModelService qua RabbitMQ là rất đúng đắn
%     \item Nó giúp đạt được AC3: Performance (<500ms) bằng cách cho phép xử lý bất đồng bộ các mô hình ML/AI (cập nhật LearnerModel)
% \end{itemize}

% \noindent\textbf{Polyglot Programming (ADR-1)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Tài liệu Hóa (Documentation)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Chiến lược Giám sát (Monitoring Strategy)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \noindent\textbf{Chiến lược Kiểm thử (Testing Strategy)}:
% \begin{itemize}[leftmargin=1.5em]
...
% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Triển khai Circuit Breakers}: Hiện tại chúng ta dựa vào K8s/Istio. Cần triển khai thêm ở cấp độ ứng dụng (ví dụ: Resilience4j cho Java, Hystrix-go cho Go) để tăng khả năng chịu lỗi
%     \item \textbf{Thêm Contract Testing}: Triển khai Pact để xác minh API giữa các service
%     \item \textbf{Tối ưu hóa CSDL}: Truy vết và sửa các truy vấn N+1 trong các service Java
%     \item \textbf{Triển khai Feature Toggles (Cờ Tính năng)}: Cho phép bật/tắt tính năng mới (ví dụ: thuật toán AI V3) cho một nhóm người dùng mà không cần deploy lại
%     \item \textbf{Tăng cường Giám sát}: Thêm các business metrics (số liệu nghiệp vụ) tùy chỉnh vào Prometheus (ví dụ: số lượng bài nộp/phút, độ trễ p99 của thuật toán AI)
% \end{itemize}

% \subsubsection{Dài hạn (6-12 tháng)}

% \begin{itemize}[leftmargin=1.5em]
%     \item \textbf{Phát triển lên Serverless}: Một số tác vụ (như ScoringService) có tải không liên tục (sporadic loads). Chuyển các hàm này sang AWS Lambda / Google Cloud Functions để giảm chi phí vận hành (idle cost) và tự động scale-to-zero
%     \item \textbf{Quản lý Mô hình AI (MLOps)}:
%     \begin{itemize}
...
%     \end{itemize}
%     \item \textbf{Hỗ trợ Đa Người thuê (Multi-tenancy)}:
%     \begin{itemize}
...
% Ghi lại các nợ kỹ thuật đã biết để lên kế hoạch xử lý.
...
% \hline
% \textbf{Hạng mục Nợ} & \textbf{Tác động} & \textbf{Ưu tiên} & \textbf{Nỗ lực} & \textbf{Kế hoạch Xử lý} \\
% \hline
