...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{1.1cm}|>{\centering\arraybackslash}p{3.1cm}|Y|Y|Y|}
        ...
        \hline
        AC-1 & Modularity        & Định nghĩa: Phân rã hệ thống thành các module độc lập với low coupling, high cohesion.\newline Tầm quan trọng: Bảo đảm live AI model swapping không downtime, cho phép cập nhật thuật toán liên tục. & Metrics: $I \approx 0$, $C_{\text{e}} < 5$, LCOM $< 1$.\newline Mục tiêu: Thay module $< 5$ phút, zero downtime khi swap AI, $\leq 3$ dependencies/module lõi.                                                     & ArchUnit Test: Fail nếu domain phụ thuộc repository cụ thể.\newline Dependency Check: Fail khi build phát hiện circular dependencies.                        \\
        \hline
        AC-2 & Scalability       & Định nghĩa: Đáp ứng tải tăng bằng horizontal scaling mà không tái thiết kế kiến trúc.\newline Tầm quan trọng: Phục vụ người học tăng trưởng theo khu vực, giờ cao điểm.                              & Metrics: Concurrent users, auto-scaling response time, CPU/Memory per request.\newline Mục tiêu: $\geq 5{,}000$ concurrent users, auto-scale $< 60$s, tăng gấp đôi resource $\rightarrow$ gấp đôi throughput.      & K6 Load Test: Staging đạt p99 $< 1$s, error rate $< 0.1\%$ tại $5{,}000$ users.\newline HPA Monitoring: Alert nếu CPU $> 70\%$ trong $> 30$s mà không scale. \\
        \hline
        AC-3 & Performance       & Định nghĩa: Đáp ứng yêu cầu nhanh để giữ trải nghiệm học tập mượt mà.\newline Tầm quan trọng: Critical cho phản hồi remedial real-time trong phiên 1-kèm-1.                                          & Metrics: Response time percentiles, throughput, cache hit ratio, query time.\newline Mục tiêu: p95 $< 500$ms, heavy ops p99 $< 1.5$s, cache hit $> 90\%$, $95\%$ query $< 100$ms.                                  & CI Latency Test: Fail nếu /api/feedback p95 $> 500$ms.\newline Performance Budget: Alert khi page load $> 2$s hoặc TTI $> 3$s.                               \\
        \hline
        AC-4 & Testability       & Định nghĩa: Dễ xác minh tính đúng đắn của AI và business logic phức tạp.\newline Tầm quan trọng: Bảo đảm độ tin cậy cho adaptive learning và scoring engine.                                         & Metrics: Code coverage, test execution time, test cases/feature, cyclomatic complexity.\newline Mục tiêu: Unit coverage $\geq 80\%$, integration $\geq 70\%$, full E2E cho critical paths, test suite $< 10$ phút. & Coverage Gate: Fail nếu ScoringEngine hoặc AdaptiveEngine $< 80\%$.\newline Mutation Testing: Mutation score $> 70\%$ cho module trọng yếu.                  \\
        \hline
    \end{tabularx}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{1.1cm}|>{\centering\arraybackslash}p{3.3cm}|Y|Y|Y|}
        ...
        \textbf{AC}   & \textbf{Đặc tính} & {\centering\textbf{Định nghĩa \& Tầm quan trọng}\par}                                                                                                                                       & {\centering\textbf{Metrics \& Mục tiêu}\par}                                                                                                                                                                 & {\centering\textbf{Fitness Function}\par}                                                                                                                                    \\
        \hline
        AC-5 & Deployability     & Định nghĩa: Khả năng triển khai phiên bản mới nhanh và độc lập.\newline Tầm quan trọng: Hỗ trợ continuous delivery, live AI model swapping mà không gián đoạn người dùng. & Metrics: Deployment frequency, lead time, rollback time, success rate.\newline Mục tiêu: Deploy $< 15$ phút, rollback $< 5$ phút, zero downtime, success rate $> 95\%$.                    & Canary Deployment Test: 1\% user, tiếp tục nếu error rate $=0\%$ sau 5 phút.\newline Health Check: Tự động rollback nếu health endpoint fail trong 2 phút. \\
        \hline
        AC-6 & Security          & Định nghĩa: Bảo vệ dữ liệu và hệ thống khỏi các mối đe doạ.\newline Tầm quan trọng: Đảm bảo an toàn PII, kết quả học tập, nội dung bản quyền.                             & Metrics: Critical vulnerabilities, patch time, auth failure rate, encryption coverage.\newline Mục tiêu: Zero critical vulns, TLS~1.3/AES-256, MFA, vá lỗi $< 24$ giờ.                     & OWASP Dependency Check: Fail build nếu phát hiện critical vulnerability.\newline Penetration Test: Kiểm thử tự động hàng quý, zero critical findings.      \\
        \hline
        AC-7 & Maintainability   & Định nghĩa: Dễ hiểu, sửa đổi, cải tiến hệ thống.\newline Tầm quan trọng: Giảm chi phí vận hành, tăng tốc fix bug và bổ sung tính năng.                                    & Metrics: Cyclomatic complexity, technical debt, MTTR, code duplication.\newline Mục tiêu: Complexity $< 10$ (90\% methods), debt $< 5\%$, duplication $< 3\%$, MTTR $< 4$ giờ.             & SonarQube Quality Gate: Fail nếu complexity $> 10$ hoặc duplication $> 5\%$.\newline Code Review Requirement: Merge cần $\geq 2$ approvals.                \\
        \hline
        AC-8 & Extensibility     & Định nghĩa: Thêm tính năng mới mà không sửa core code.\newline Tầm quan trọng: Thích ứng với nhu cầu giáo dục và công nghệ mới.                                           & Metrics: Extension points, time to add feature, breaking changes.\newline Mục tiêu: Plugin cho content types, thêm assessment mới không chạm core, breaking changes $< 5\%$/major release. & OCP Compliance Test: Xác nhận mở rộng qua config/plugins.\newline API Compatibility: Test tự động bảo đảm backward compatibility.                          \\
        \hline
        AC-9 & Observability     & Định nghĩa: Hiểu trạng thái hệ thống qua logs, metrics, traces.\newline Tầm quan trọng: Phát hiện sớm sự cố trong hệ thống phân tán.                                      & Metrics: Log coverage, MTTD, dashboard coverage, alert accuracy.\newline Mục tiêu: 100\% requests có trace ID, MTTD $< 5$ phút, đủ dashboards, alert noise $< 10\%$.                       & Trace Coverage Test: Alert nếu $< 95\%$ requests có trace ID.\newline SLO Monitoring: Tự động on-call khi SLO breach detected.                             \\
        \hline
    \end{tabularx}
