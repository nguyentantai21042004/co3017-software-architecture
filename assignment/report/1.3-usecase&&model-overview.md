...
**Mục tiêu:** Xác định **ranh giới (boundaries)** dựa trên ngữ nghĩa nghiệp vụ để tạo cơ sở cho kiến trúc Microservices (AC1: Modularity).

**Ví dụ cụ thể:**

| **Khối Domain** | **Vấn đề Nghiệp vụ** | **Hành động Kiến trúc** | **Kết quả** |
|----------------|---------------------|------------------------|-------------|
| **LearnerModelAggregate** | - Cần cập nhật liên tục (mỗi lần làm bài)<br>- Tính toán AI nặng (BKT algorithm)<br>- Thay đổi logic không ảnh hưởng profile | Tách thành **LearnerModel Service** riêng | ✅ Deploy AI độc lập (FR12)<br>✅ Scale riêng khi nhiều user (AC2) |
| **ContentAggregate** | - Dữ liệu ít thay đổi<br>- Nhiều service phụ thuộc (Engine, Dashboard, Feedback) | Tách thành **Content Management Service** với I≈0 | ✅ Reliability cao (AC7)<br>✅ Caching hiệu quả |
| **AdaptivePathGenerator** | - Cần hoán đổi thuật toán AI dễ dàng (FR9/AC1) | Đặt trong **Adaptive Engine Service** độc lập | ✅ Blue/Green deployment<br>✅ A/B testing algorithms |
...
**Minh họa DIP trong ITS:**

```
┌─────────────────────────────────────┐
│   AdaptivePathGenerator (Policy)   │  ← I ≈ 0 (Stable)
│   - generatePath(learnerId)         │
└─────────────────┬───────────────────┘
                  │ depends on
                  ▼
    ┌──────────────────────────────┐
    │ LearnerModelRepository (I)   │  ← Interface (Abstraction)
    └──────────────────────────────┘
                  △
                  │ implements
                  │
    ┌──────────────────────────────┐
    │ PostgresLearnerModelRepo     │  ← I → 1 (Unstable Detail)
    └──────────────────────────────┘
```

**Lợi ích:**
- ✅ Test `AdaptivePathGenerator` WITHOUT database (mock interface)
- ✅ Swap PostgreSQL → MongoDB không sửa Policy code
- ✅ Policy stable (I≈0), Detail volatile (I→1)
...
**B. Ví dụ Code Test (Golang)**

```go
// Domain Service (Policy)
type ScoringEngine struct {
    assessmentRepo AssessmentRepository // Interface
}

func (s *ScoringEngine) CalculateScore(submissionID string) (float64, error) {
    assessment, _ := s.assessmentRepo.FindBySubmission(submissionID)
    // Pure business logic - easy to test
    return computeScore(assessment.GetCorrectAnswers(), assessment.GetUserAnswers()), nil
}

// Unit Test (NO DATABASE)
func TestScoringEngine(t *testing.T) {
    // Mock repository
    mockRepo := &MockAssessmentRepo{
        FindBySubmissionFunc: func(id string) (*Assessment, error) {
            return &Assessment{CorrectAnswers: []string{"A", "B"}, UserAnswers: []string{"A", "B"}}, nil
        },
    }
    
    engine := &ScoringEngine{assessmentRepo: mockRepo}
    score, _ := engine.CalculateScore("sub-123")
    
    assert.Equal(t, 100.0, score) // Test passed WITHOUT DB!
}
```

**Kết quả:**
- ✅ Test tốc độ cao (ms thay vì giây)
...
| **AdaptiveEngine** | 2 (Learner Dashboard, UC-08 flow) | 2 (LearnerModel, Content) | 2/(2+2) = **0.5** | ⚖️ Vừa phải - Policy module, nhưng cần đọc data từ 2 services. |
| **LearnerModel** | 3 (Engine, Feedback, Dashboard) | 1 (Content for tags) | 1/(1+3) = **0.25** | ✅ Tương đối ổn định - Core state nhiều service cần. |
| **ScoringService** | 1 (Learner submit) | 3 (Assessment, LearnerModel, Feedback) | 3/(3+1) = **0.75** | ⚠️ Bất ổn - Orchestrator phụ thuộc nhiều service, logic thay đổi nhiều. |

**Quyết định Kiến trúc dựa trên I:**

```
┌─────────────────────────────────────────┐
│   Nguyên tắc: I≈0 (Stable) không phụ   │
│   thuộc vào I→1 (Unstable)              │
│            ↓ DIP ↓                       │
└─────────────────────────────────────────┘

✅ ĐÚNG:
AdaptiveEngine (I=0.5) → ContentService (I=0)
       (Policy)              (Stable Detail)

❌ SAI:
ContentService (I=0) → ScoringService (I=0.75)
   (Stable)               (Unstable)
   → Sẽ làm Content bất ổn
```
...
```
┌─────────────────────────────────────────────┐
│  LearnerAggregate (Aggregate Root)          │
│  ┌───────────────────────────────────────┐  │
│  │ Learner (Root)                        │  │ ← Aggregate Root
│  │ - learnerId: UUID                     │  │
│  │ - name, email                         │  │
│  └───────────────────────────────────────┘  │
│       │ owns (1:N)                           │
│       ▼                                      │
│  ┌───────────────────────────────────────┐  │
│  │ LearnerProfile (Entity)               │  │ ← Part of Aggregate
│  │ - profileId                           │  │
│  │ - learningGoal, schedule              │  │
│  └───────────────────────────────────────┘  │
│       │ owns (1:N)                           │
│       ▼                                      │
│  ┌───────────────────────────────────────┐  │
│  │ ProgressRecord (Value Object)         │  │ ← Part of Aggregate
│  │ - courseId, completion%               │  │
│  └───────────────────────────────────────┘  │
└─────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────┐
│  LearnerModelAggregate (DIFFERENT TX)       │ ← Separate Aggregate
│  - SkillMasteryScore                        │
└─────────────────────────────────────────────┘
```
...
##### Ví dụ Transaction Boundary (Code)

**✅ ĐÚNG: Single Aggregate Transaction**

```java
// Update LearnerProfile (within LearnerAggregate)
@Transactional
public void updateLearningGoal(UUID learnerId, String newGoal) {
    Learner learner = learnerRepo.findById(learnerId); // Aggregate Root
    learner.getProfile().setLearningGoal(newGoal);     // Update child entity
    learnerRepo.save(learner);                         // Single TX
}
```

**❌ SAI: Cross-Aggregate Transaction (Distributed Transaction - tránh!)**

```java
// ❌ BAD: Update 2 Aggregates in 1 TX
@Transactional
public void updateProfileAndSkills(UUID learnerId, ...) {
    Learner learner = learnerRepo.findById(learnerId);      // Aggregate 1
    learner.getProfile().setGoal(newGoal);
    
    LearnerModel model = modelRepo.findById(learnerId);     // Aggregate 2
    model.updateSkill("Algebra", 0.9);                      // CROSS-AGGREGATE!
    
    // Both updates in 1 TX → Tight coupling, deadlock risk
}
```

**✅ ĐÚNG: Event-Driven Cross-Aggregate Update**
...
public void updateLearningGoal(UUID learnerId, String newGoal) {
    ...
    learner.getProfile().setLearningGoal(newGoal);
    learnerRepo.save(learner);
    
    // Emit event for other services
    eventPublisher.publish(new LearningGoalUpdatedEvent(learnerId, newGoal));
}

// LearnerModel Service listens to event
@EventListener
public void onLearningGoalUpdated(LearningGoalUpdatedEvent event) {
    // Update LearnerModel in separate TX (Eventual Consistency)
    modelService.recalculatePath(event.getLearnerId());
}
```
...
- Ví dụ: `Learner.updateEmail()` nên ở trong `Learner` class, không cần service
